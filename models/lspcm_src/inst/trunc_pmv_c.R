...
# open the pdf file
cairo_pdf("~/lspm/inst/extdata/adapt/figure/trunc_pmv_c.pdf", width = 10, height = 4)
...
vioplot(cbind(node100results4d3_adapt_c[[paste0("seed",(seed_number[2]))]]$mcmc_chain$"8D"$variances[complete.cases(node100results4d3_adapt_c$seed630678$mcmc_chain$"8D"$variances),,drop=F]),
...
for(seed in seed_number) {
  lapply(node100results4d3_adapt_c[[paste0("seed",(seed))]]$mcmc_chain, function(x) vioplot(x$variances[complete.cases(x$variances),,drop=F], add=T, cex=0.2, rectCol = NA, lineCol = NA, cex.lab=1.5, cex.main=1.5, cex.axis=1.5,
...
#   vioplot(node100results4d3_adapt_c[[paste0("seed",(seed))]]$mcmc_chain$"4D"$variances, add=TRUE, cex=0.2, rectCol = NA, lineCol = NA,
#           colMed = NA, border=NA, col=rgb(red = 0, green=0, blue=0,alpha=0.03))
# }
points(1/cumprod(c(0.5,1.1,1.05,1.15)), col='red', pch=4, cex=2, lwd=3) # true
...
for(seed in seed_number) {
  lapply(node100results4d3_adapt_low_d_c[[paste0("seed",(seed))]]$mcmc_chain, function(x) vioplot(x$variances[complete.cases(x$variances),,drop=F], add=T, cex=0.2, rectCol = NA, lineCol = NA, cex.lab=1.5, cex.main=1.5, cex.axis=1.5,
                                                                                                colMed = NA, border=NA, col=rgb(red = 0, green=0, blue=0,alpha=0.03)))
}
...
for(seed in seed_number) {
  lapply(node100results4d3_adapt_same_d_c[[paste0("seed",(seed))]]$mcmc_chain, function(x) vioplot(x$variances[complete.cases(x$variances),,drop=F], add=T, cex=0.2, rectCol = NA, lineCol = NA, cex.lab=1.5, cex.main=1.5, cex.axis=1.5,
                                                                                                 colMed = NA, border=NA, col=rgb(red = 0, green=0, blue=0,alpha=0.03)))
}
title(xlab=expression(paste("Dimension \u2113")), cex.lab=1.5, line=2.5)
title(ylab=expression(paste("Variance ", omega["\u2113"]^{-1})), line=2.2)
...
for(seed in seed_number) {
  lapply(node100results4d3_adapt_high_d_c[[paste0("seed",(seed))]]$mcmc_chain, function(x) vioplot(x$variances[complete.cases(x$variances),,drop=F], add=T, cex=0.2, rectCol = NA, lineCol = NA, cex.lab=1.5, cex.main=1.5, cex.axis=1.5,
                                                                                                 colMed = NA, border=NA, col=rgb(red = 0, green=0, blue=0,alpha=0.03)))
}
