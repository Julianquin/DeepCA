...
# open the pdf file
cairo_pdf("~/lspm/inst/extdata/adapt/figure/size_pmd_c.pdf", width = 10, height = 4)
...
vioplot(cbind(node200results_adapt_c[[paste0("seed",(seed_number[1]))]]$mcmc_chain$"4D"$deltas[complete.cases(node200results_adapt_c[[paste0("seed",(seed_number[1]))]]$mcmc_chain$"4D"$deltas),,drop=F]),
        ylim=c(0,40), cex=0.2, rectCol = NA, lineCol = NA, cex.lab=1.5, cex.main=1.5, cex.axis=1.5,
        colMed = NA, border=NA, col=rgb(red = 0, green=0, blue=0,alpha=0),
        main= "n = 20", #main='Posterior Mean Variance vs Dimension',
        xlab= "", ylab="")
for(seed in seed_number) {
  lapply(node20results_adapt_c[[paste0("seed",(seed))]]$mcmc_chain, function(x) vioplot(x$deltas[complete.cases(x$deltas),,drop=F], add=T, cex=0.2, rectCol = NA, lineCol = NA, cex.lab=1.5, cex.main=1.5, cex.axis=1.5,
...
vioplot(cbind(node200results_adapt_c[[paste0("seed",(seed_number[1]))]]$mcmc_chain$"4D"$deltas[complete.cases(node200results_adapt_c[[paste0("seed",(seed_number[1]))]]$mcmc_chain$"4D"$deltas),,drop=F]),
        ...
        xlab= "", ylab="", yaxt="n")
for(seed in seed_number) {
  lapply(node50results_adapt_c[[paste0("seed",(seed))]]$mcmc_chain, function(x) vioplot(x$deltas[complete.cases(x$deltas),,drop=F], add=T, cex=0.2, rectCol = NA, lineCol = NA, cex.lab=1.5, cex.main=1.5, cex.axis=1.5,
                                                                                      colMed = NA, border=NA, col=rgb(red = 0, green=0, blue=0,alpha=0.03)))
}
...
vioplot(cbind(node200results_adapt_c[[paste0("seed",(seed_number[1]))]]$mcmc_chain$"4D"$deltas[complete.cases(node200results_adapt_c[[paste0("seed",(seed_number[1]))]]$mcmc_chain$"4D"$deltas),,drop=F]),
        ...
        xlab= "", ylab="", yaxt="n")
for(seed in seed_number) {
  lapply(node100results_adapt_c[[paste0("seed",(seed))]]$mcmc_chain, function(x) vioplot(x$deltas[complete.cases(x$deltas),,drop=F], add=T, cex=0.2, rectCol = NA, lineCol = NA, cex.lab=1.5, cex.main=1.5, cex.axis=1.5,
                                                                                       colMed = NA, border=NA, col=rgb(red = 0, green=0, blue=0,alpha=0.03)))
}
title(xlab=expression(paste("Dimension h")), cex.lab=1.5, line=2.5)
title(ylab=expression(paste("Shrinkage strength ", delta[h])), line=2.2)
...
vioplot(cbind(node200results_adapt_c[[paste0("seed",(seed_number[1]))]]$mcmc_chain$"4D"$deltas[complete.cases(node200results_adapt_c[[paste0("seed",(seed_number[1]))]]$mcmc_chain$"4D"$deltas),,drop=F]),
        ...
        xlab= "", ylab="", yaxt="n")
for(seed in seed_number) {
  lapply(node200results_adapt_c[[paste0("seed",(seed))]]$mcmc_chain, function(x) vioplot(x$deltas[complete.cases(x$deltas),,drop=F], add=T, cex=0.2, rectCol = NA, lineCol = NA, cex.lab=1.5, cex.main=1.5, cex.axis=1.5,
                                                                                       colMed = NA, border=NA, col=rgb(red = 0, green=0, blue=0,alpha=0.03)))
}
