...
# open the pdf file
cairo_pdf("~/lspm/inst/extdata/adapt/figure/trunc_alpha_c.pdf", width = 5, height = 3)
...
boxplot(tempalpha, xlab=expression(p[0]) ,ylab=expression(paste("", alpha)), ylim=c(4,6.5), boxwex=0.4, xaxt="n", cex.lab=1.5)
...
for(d in 2:5){
  for(seed in seed_number) {
    if(names(which.max(table(node100results4d3_adapt_c[[paste0("seed",(seed))]]$iter_d))) != d){
      ...
      alpha_data = node100results4d3_adapt_c[[paste0("seed",(seed))]]$alpha[node100results4d3_adapt_c[[paste0("seed",(seed))]]$iter_d == d]
...
for(d in 2:5){
  for(seed in seed_number[1:18]) {
    if(names(which.max(table(node100results4d3_adapt_low_d_c[[paste0("seed",(seed))]]$iter_d))) != d){
      next
    } else{
      alpha_data = node100results4d3_adapt_low_d_c[[paste0("seed",(seed))]]$alpha[node100results4d3_adapt_low_d_c[[paste0("seed",(seed))]]$iter_d == d]
...
for(d in 2:5){
  for(seed in seed_number[1:10]) {
    if(names(which.max(table(node100results4d3_adapt_same_d_c[[paste0("seed",(seed))]]$iter_d))) != d){
      ...
      alpha_data = node100results4d3_adapt_same_d_c[[paste0("seed",(seed))]]$alpha[node100results4d3_adapt_same_d_c[[paste0("seed",(seed))]]$iter_d == d]
...
for(d in 2:5){
  for(seed in seed_number[1:20]) {
    if(names(which.max(table(node100results4d3_adapt_high_d_c[[paste0("seed",(seed))]]$iter_d))) != d){
      next
    } else{
      alpha_data = node100results4d3_adapt_high_d_c[[paste0("seed",(seed))]]$alpha[node100results4d3_adapt_high_d_c[[paste0("seed",(seed))]]$iter_d == d]
