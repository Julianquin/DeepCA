...
# open the pdf file
cairo_pdf("~/lspm/inst/extdata/adapt/figure/size_alpha_c.pdf", width = 5, height = 4)
...
boxplot(tempalpha, xlab=expression(n) ,ylab=expression(paste("", alpha)), ylim=c(2.5,3.5), boxwex=0.4, xaxt="n")
...
for(d in 2:4){
  for(seed in seed_number) {
    if(names(which.max(table(node20results_adapt_c[[paste0("seed",(seed))]]$iter_d))) != d){
      ...
      alpha_data = node20results_adapt_c[[paste0("seed",(seed))]]$alpha[node20results_adapt_c[[paste0("seed",(seed))]]$iter_d == d]
...
for(d in 2:4){
  for(seed in seed_number) {
    if(names(which.max(table(node50results_adapt_c[[paste0("seed",(seed))]]$iter_d))) != d){
      ...
      alpha_data = node50results_adapt_c[[paste0("seed",(seed))]]$alpha[node50results_adapt_c[[paste0("seed",(seed))]]$iter_d == d]
...
for(d in 2:4){
  for(seed in seed_number) {
    if(names(which.max(table(node100results_adapt_c[[paste0("seed",(seed))]]$iter_d))) != d){
      ...
      alpha_data = node100results_adapt_c[[paste0("seed",(seed))]]$alpha[node100results_adapt_c[[paste0("seed",(seed))]]$iter_d == d]
...
for(d in 2:4){
  for(seed in seed_number) {
    if(names(which.max(table(node200results_adapt_c[[paste0("seed",(seed))]]$iter_d))) != d){
      ...
      alpha_data = node200results_adapt_c[[paste0("seed",(seed))]]$alpha[node200results_adapt_c[[paste0("seed",(seed))]]$iter_d == d]
